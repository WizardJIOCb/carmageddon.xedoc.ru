...
const shieldIds = new Set(['none','deflector','aegis']);
...
const roomSnapshot = room => ({
  ...
  players: [...room.players.values()].map(player => ({ id: player.id, name: player.name, carId: player.carId, color: player.color, machineGunId:player.machineGunId, missileLauncherId:player.missileLauncherId, shieldId:player.shieldId })),
});
...
wss.on('connection', socket => {
  ...
  socket.on('message', raw => {
    ...
    if (message.type === 'combat' && room.status === 'racing') {
      ...
      if(kind==='ai_damage'){if(client.id===room.hostId)return;const targetIndex=Math.trunc(clamp(event.targetIndex,0,Math.max(0,room.aiSlots-1))),damage=clamp(event.damage,0,60),position=vector(event.position),direction=vector(event.direction),host=room.players.get(room.hostId);if(!host||damage<=0)return;send(host.socket,{type:'combat_event',playerId:client.id,event:{kind,targetIndex,damage,position,direction}});return;}
