...
class WreckrunGame {
  ...
  updateParticles(dt){for(let i=this.particles.length-1;i>=0;i--){const p=this.particles[i];p.userData.life-=dt;p.userData.vel.y-=(p.userData.gravity??(p.isSprite?.45:2.5))*dt;p.position.addScaledVector(p.userData.vel,dt);if(p.userData.bloodDrop&&p.position.y<=.07&&p.userData.vel.y<0){if(p.userData.splat)this.spawnBloodDecal(p.position,p.userData.vel,.16+Math.random()*.22,13+Math.random()*7);p.userData.life=0;}p.material.opacity=Math.max(0,p.userData.life/p.userData.maxLife)*(p.userData.baseOpacity??1);if(!p.userData.bloodDrop)p.scale.addScalar(dt*(p.userData.growth??(p.isSprite?1.15:.35)));if(p.userData.life<=0){this.scene.remove(p);if(!p.userData.sharedGeometry)p.geometry?.dispose();p.material?.dispose();this.particles.splice(i,1);}}}
