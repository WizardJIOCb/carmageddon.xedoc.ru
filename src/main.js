...
class WreckrunGame {
  ...
  makeObstacle(x,z){
    ...
    const rotation={x:0,y:Math.sin(yaw*.5),z:0,w:Math.cos(yaw*.5)},body=this.physics.createRigidBody(RAPIER.RigidBodyDesc.dynamic().setTranslation(visual.position.x,visual.position.y,visual.position.z).setRotation(rotation).setLinearDamping(wide?.4:.32).setAngularDamping(wide?.9:1.05).setCcdEnabled(false));
    ...
    const mass=wide?14+scale*6:2.2+scale*1.4,collider=this.physics.createCollider(desc.setMass(mass).setFriction(wide?.68:.58).setFrictionCombineRule(RAPIER.CoefficientCombineRule.Min).setRestitution(wide?.035:.05).setActiveEvents(RAPIER.ActiveEvents.CONTACT_FORCE_EVENTS).setContactForceEventThreshold(600),body);
    const obstacle={visual,body,collider,type:wide?'crate':'cone',size,mass,health:wide?2.8:1,breakForce:wide?15000:4200,lastImpact:-10,broken:false,pendingBreak:false};
  ...
  ragdollPedestrian(p,impactDirection=null,impactStrength=null,applyRecoil=true){
    if(p.userData.dead)return;p.userData.dead=true;this.kills++;const f=impactDirection?.clone().setY(0).normalize()||forwardVector(this.player.userData.vehicle.body,new THREE.Vector3()),speed=Math.max(8,Math.abs(this.player.userData.speed)),model=p.userData.model;p.userData.mixer.stopAllAction();p.updateMatrixWorld(true);model.updateMatrixWorld(true);this.scene.attach(model);p.visible=false;
      const strength=Math.min(12,impactStrength??(3.6+speed*.32)),impulse=f.clone().multiplyScalar(strength);impulse.y=impactStrength?Math.min(4.6,strength*.28):3.2+Math.min(2.1,speed*.07);this.ragdolls.push(createRagdoll(this.physics,this.scene,p.position,impulse,{skin:p.userData.skin,cloth:p.userData.cloth},model));
  ...
  handlePhysicsContacts(){
    const breaks=[];
    this.eventQueue.drainCollisionEvents((h1,h2,started)=>{if(!started)return;const obstacle1=this.colliderDestructibles.get(h1),obstacle2=this.colliderDestructibles.get(h2),car1=this.colliderVehicles.get(h1),car2=this.colliderVehicles.get(h2),queuePole=(obstacle,car)=>{if(!obstacle||obstacle.type!=='pole'||obstacle.pendingBreak||!car)return;obstacle.pendingBreak=true;const direction=forwardVector(car.userData.vehicle.body,new THREE.Vector3());breaks.push({obstacle,direction,source:car});};queuePole(obstacle1,car2);queuePole(obstacle2,car1);});
  ...
  breakObstacle(obstacle,direction,source){
    ...
    for(let i=0;i<count;i++){
      ...
      const mass=isPole?.6+Math.random()*.5:1.1+Math.random()*2.2,body=this.physics.createRigidBody(RAPIER.RigidBodyDesc.dynamic().setTranslation(visual.position.x,visual.position.y,visual.position.z).setLinearDamping(isPole?1.8:.28).setAngularDamping(isPole?5.2:.75).setCcdEnabled(false)),collider=isPole?RAPIER.ColliderDesc.cuboid(Math.max(.045,chunkScale*.28),Math.max(.12,chunkScale*1.4),Math.max(.045,chunkScale*.28)):RAPIER.ColliderDesc.ball(chunkScale*.65);if(isPole)collider.setCollisionGroups(0x00020001);this.physics.createCollider(collider.setMass(mass).setFriction(isPole?.92:1.05).setFrictionCombineRule(RAPIER.CoefficientCombineRule.Max).setRestitution(isPole?.005:.08),body);
