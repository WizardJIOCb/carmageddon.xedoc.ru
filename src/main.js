...
class WreckrunGame {
  ...
  destroy(){if(this.destroyed)return;this.destroyed=true;cancelAnimationFrame(this.raf);removeEventListener('keydown',this.onKeyDown);removeEventListener('keyup',this.onKeyUp);removeEventListener('resize',this.onResize);this.renderer?.domElement.removeEventListener('pointerdown',this.onPointerDown);this.renderer?.domElement.removeEventListener('pointermove',this.onPointerMove);this.renderer?.domElement.removeEventListener('pointerup',this.onPointerUp);this.renderer?.domElement.removeEventListener('pointercancel',this.onPointerUp);this.eventQueue?.free();this.physics?.free();this.renderer?.dispose();this.renderer?.domElement.remove();this.hud?.remove();}
}
...
function createPhysicsMenuDemo(container){
  ...
  const resize=()=>{const w=Math.max(1,container.clientWidth),h=Math.max(1,container.clientHeight);renderer.setSize(w,h,false);camera.aspect=w/h;camera.updateProjectionMatrix();},observer=new ResizeObserver(resize);observer.observe(container);resize();
  const clearSimulation=()=>{for(const car of cars)scene.remove(car.visual.group);for(const p of people){scene.remove(p.group);scene.remove(p.person.model);}for(const ragdoll of ragdolls)scene.remove(ragdoll.model);for(const effect of effects){if(effect.sprite)scene.remove(effect.sprite);if(effect.light&&!effect.pooledLight)scene.remove(effect.light);effect.sprite?.material?.dispose();}for(const light of menuLights)light.intensity=0;cars=[];people=[];ragdolls=[];effects=[];deferredEffects=[];eventQueue?.free();world?.free();eventQueue=null;world=null;};
