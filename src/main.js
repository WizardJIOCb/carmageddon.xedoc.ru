...
const DEFAULT_SETTINGS = {
  ...
  invertCameraX: false,
  graphics: 'high',
  bindings: { accelerate:'KeyW', brake:'KeyS', left:'KeyA', right:'KeyD', fire:'Space', nitro:'ShiftLeft', reset:'KeyR', pause:'Escape' },
};
...
function loadSettings(){try{const stored=JSON.parse(localStorage.getItem('wreckrun-settings')||'{}');return{...structuredClone(DEFAULT_SETTINGS),...stored,bindings:{...DEFAULT_SETTINGS.bindings,...(stored.bindings||{})}};}catch{return structuredClone(DEFAULT_SETTINGS);}}
...
function showSettings(tab='controls'){
  ...
  const actions=[['accelerate','Газ'],['brake','Тормоз / задний ход'],['left','Поворот влево'],['right','Поворот вправо'],['fire','Огонь'],['nitro','Нитро'],['reset','Эвакуация'],['pause','Пауза']];
  let content='';
  ...
  if(tab==='about')content=`<div class="settings-section about-game"><div class="eyebrow">WRECK INDUSTRIES // DOSSIER</div><h3>Wreckrun: Aftermath</h3><p>Аркадная автомобильная бойня о гонках без правил. Побеждай кругами, уничтожением всех соперников или выполнением квоты. Машины деформируются, оружие перестраивает бой, а каждый район предлагает собственный свет, погоду и характер противников.</p><div class="about-grid"><div><small>Версия</small><strong>${GAME_VERSION}</strong></div><div><small>Движок</small><strong>THREE.JS + RAPIER</strong></div><div><small>Кампания</small><strong>5 РАЙОНОВ</strong></div><div><small>Сборка</small><strong>WEBGL DESKTOP</strong></div></div><p class="legal-note">Независимый браузерный прототип. Не является официальной игрой серии Carmageddon.</p></div>`;
...
class WreckrunGame {
  constructor(level){
    ...
    this.ragdolls=[];this.physicsDebris=[];this.destructibles=[];this.navigationObstacles=[];this.burningCars=[];this.deferredEffects=[];this.colliderVehicles=new Map();this.colliderDestructibles=new Map();this.elapsed=0;this.kills=0;this.wrecks=0;this.lap=0;this.checkpoint=0;this.ended=false;this.paused=false;this.lastShot=-10;this.shake=0;this.accumulator=0;this.playerDeathPending=false;this.deathModalAt=Infinity;this.victoryPending=false;this.victoryModalAt=Infinity;this.cameraOrbitYaw=0;this.cameraOrbitPitch=0;this.cameraDragging=false;this.lastCameraInput=-10;this.boostIntensity=0;this.boostEmit=0;
  }
  ...
  createPedestrian(i){
    const person=this.models.createPerson(i),g=new THREE.Group();g.add(person.model);g.userData={dead:false,panic:0,phase:Math.random()*10,runSpeed:4.4+Math.random()*2.2,swerve:Math.random()<.5?-1:1,model:person.model,mixer:person.mixer,idle:person.idle,run:person.run,hips:person.hips,hipsBasePosition:person.hipsBasePosition,skin:[0xd99a78,0x8a5a42,0xf0bd91][i%3],cloth:[0xbab599,0x1e6b72,0x863328,0x41404d][i%4]};return g;
  }
  ...
  animate=()=>{if(this.destroyed)return;this.raf=requestAnimationFrame(this.animate);const dt=Math.min(this.clock.getDelta(),.034);if(!this.paused&&!this.ended){this.elapsed+=dt;this.updatePlayer(dt);this.updateAI(dt);this.updatePedestrians(dt);this.physics.timestep=dt;this.physics.step(this.eventQueue);this.handlePhysicsContacts();syncVehicle(this.player.userData.vehicle);this.opponents.forEach(ai=>syncVehicle(ai.userData.vehicle));this.syncDestructibles();this.ragdolls.forEach(syncRagdoll);this.syncPhysicsDebris(dt);this.hitPedestrians();this.updateProjectiles(dt);this.updateBurningCars(dt);this.updateDeferredEffects(dt);this.updateParticles(dt);if(this.playerDeathPending&&this.elapsed>=this.deathModalAt)this.finish(false,'МАШИНА УНИЧТОЖЕНА');this.checkRules();this.updateHUD();}this.updateCamera(dt);this.renderer.render(this.scene,this.camera);};

  updatePlayer(dt){
    const d=this.player.userData;if(d.dead)return;const forward=this.actionPressed('accelerate'),reverse=this.actionPressed('brake'),left=this.actionPressed('left'),right=this.actionPressed('right');
    ...
    driveVehicle(d.vehicle,{throttle,steer,steerSensitivity:settings.steeringSensitivity,steerSmoothing:settings.steeringSmoothing,brake:!throttle&&Math.abs(d.speed)<1?.15:0,boost:boosting},dt);d.speed=d.vehicle.controller.currentVehicleSpeed();
  ...
  handlePhysicsContacts(){
    const breaks=[];
    this.eventQueue.drainCollisionEvents((h1,h2,started)=>{if(!started)return;const obstacle1=this.colliderDestructibles.get(h1),obstacle2=this.colliderDestructibles.get(h2),car1=this.colliderVehicles.get(h1),car2=this.colliderVehicles.get(h2),queueBreakable=(obstacle,car)=>{if(!obstacle||!['pole','crate'].includes(obstacle.type)||obstacle.pendingBreak||!car||Math.abs(car.userData.speed)<2)return;obstacle.pendingBreak=true;const direction=forwardVector(car.userData.vehicle.body,new THREE.Vector3());breaks.push({obstacle,direction,source:car});};queueBreakable(obstacle1,car2);queueBreakable(obstacle2,car1);});
  ...
  breakObstacle(obstacle,direction,source){
    ...
    const position=obstacle.visual.position.clone(),sourceVelocity=source?.userData.vehicle.body.linvel()||{x:direction.x*8,y:0,z:direction.z*8},isPole=obstacle.type==='pole',count=5,palette=obstacle.type==='crate'?[0x5a2d12,0x8d491d,0xc17430]:(isPole?[0x111414,0x343b3b,this.level.accent]:[0xff6518,0xf1ead4,0x24282b]);
  ...
  carExplosion(car){
    const pos=car.position.clone();this.explosion(pos,1.65);for(let i=0;i<6;i++){const direction=new THREE.Vector3((Math.random()-.5)*1.6,.2+Math.random()*.7,(Math.random()-.5)*1.6).normalize(),amount=12+Math.random()*11;this.deferEffect(.02+i*.025,()=>this.spawnDebris(car,direction,amount));}const light=new THREE.PointLight(0xff4c13,18,14,2);light.position.copy(pos).add(new THREE.Vector3(0,1.1,0));this.scene.add(light);this.burningCars.push({car,life:7.5,emit:.08,light});this.shake=1.35;
  }
  ...
  checkRules(){if(this.playerDeathPending)return;if(this.victoryPending){if(this.elapsed>=this.victoryModalAt)this.finish(true,'ВСЕ СОПЕРНИКИ УНИЧТОЖЕНЫ');return;}if(this.wrecks>=this.level.enemies){this.victoryPending=true;this.victoryModalAt=this.elapsed+3;this.addEvent('<strong>ПОСЛЕДНИЙ СОПЕРНИК УНИЧТОЖЕН</strong> Подтверждение победы...');return;}if(this.lap>=this.level.laps)this.finish(true,'ГОНКА ЗАВЕРШЕНА');else if(this.kills>=this.level.quota)this.finish(true,'КВОТА ВЫПОЛНЕНА');}
  ...
  updateDeferredEffects(dt){let budget=3;for(let i=this.deferredEffects.length-1;i>=0;i--){const effect=this.deferredEffects[i];effect.delay-=dt;if(effect.delay>0||budget<=0)continue;this.deferredEffects.splice(i,1);effect.callback();budget--;}}
  explosion(pos,intensity=1){
    ...
    scheduleBatches(sparkCount,10,()=>this.emitParticle(pos.clone().add(new THREE.Vector3(0,.5,0)),Math.random()<.7?0xffb21a:0xffffff,.035+Math.random()*.055,.25+Math.random()*.55,new THREE.Vector3((Math.random()-.5)*12,2+Math.random()*7,(Math.random()-.5)*12)),.02);
    const light=new THREE.PointLight(0xff4317,42*intensity,30*intensity,2);light.position.copy(pos).add(new THREE.Vector3(0,2,0));this.scene.add(light);this.deferEffect(.3,()=>this.scene?.remove(light));this.shake=Math.max(this.shake,intensity*1.15);
  }
  ...
  updateParticles(dt){for(let i=this.particles.length-1;i>=0;i--){const p=this.particles[i];p.userData.life-=dt;p.userData.vel.y-=(p.userData.gravity??(p.isSprite?.45:2.5))*dt;p.position.addScaledVector(p.userData.vel,dt);p.material.opacity=Math.max(0,p.userData.life/p.userData.maxLife)*(p.userData.baseOpacity??1);p.scale.addScalar(dt*(p.userData.growth??(p.isSprite?1.15:.35)));if(p.userData.life<=0){this.scene.remove(p);if(!p.userData.sharedGeometry)p.geometry?.dispose();p.material?.dispose();this.particles.splice(i,1);}}}
  ...
  updateCamera(dt){
    if(!this.player)return;if(!this.cameraDragging&&this.elapsed-this.lastCameraInput>2){this.cameraOrbitYaw=THREE.MathUtils.damp(this.cameraOrbitYaw,0,2.1,dt);this.cameraOrbitPitch=THREE.MathUtils.damp(this.cameraOrbitPitch,0,2.1,dt);}const speed=Math.abs(this.player.userData.speed),distance=10.5+speed*.08+this.boostIntensity*1.8,height=5.8+speed*.035+this.cameraOrbitPitch*6+this.boostIntensity*.45,offset=new THREE.Vector3(Math.sin(this.cameraOrbitYaw)*distance,height,-Math.cos(this.cameraOrbitYaw)*distance).applyQuaternion(this.player.quaternion),desired=this.player.position.clone().add(offset);if(this.boostIntensity>.05){desired.x+=(Math.random()-.5)*.045*this.boostIntensity;desired.y+=(Math.random()-.5)*.035*this.boostIntensity;}if(this.shake>0){desired.x+=(Math.random()-.5)*this.shake;desired.y+=(Math.random()-.5)*this.shake;this.shake=Math.max(0,this.shake-dt*2.8);}this.camera.position.lerp(desired,1-Math.pow(.002,dt));const orbitSide=Math.abs(Math.sin(this.cameraOrbitYaw)),lookAhead=speed*.18*(1-orbitSide*.72),look=this.player.position.clone().add(new THREE.Vector3(0,1+this.cameraOrbitPitch*.45,0)).add(new THREE.Vector3(0,0,lookAhead).applyQuaternion(this.player.quaternion));this.camera.lookAt(look);this.camera.fov=THREE.MathUtils.damp(this.camera.fov,62+Math.min(13,speed*.25)+this.boostIntensity*7,4.2,dt);this.camera.updateProjectionMatrix();this.renderer.domElement.dataset.cameraOrbit=this.cameraOrbitYaw.toFixed(3);this.renderer.domElement.dataset.boost=this.boostIntensity.toFixed(2);
  }
  ...
  updateHUD(){
    ...
    const mapX=x=>THREE.MathUtils.clamp(50+x/1.75,4,96),mapZ=z=>THREE.MathUtils.clamp(50+z/1.45,4,96),playerMarker=this.hud.querySelector('.map-player'),playerYaw=new THREE.Euler().setFromQuaternion(this.player.quaternion,'YXZ').y;if(playerMarker){playerMarker.style.left=mapX(this.player.position.x)+'%';playerMarker.style.top=mapZ(this.player.position.z)+'%';playerMarker.style.transform=`translate(-50%,-50%) rotate(${playerYaw}rad)`;}this.opponents.forEach((o,i)=>{const el=this.hud.querySelector(`[data-map-enemy="${i}"]`);if(!el)return;if(o.userData.dead){el.style.display='none';return;}el.style.left=mapX(o.position.x)+'%';el.style.top=mapZ(o.position.z)+'%';});
  }
  ...
  destroy(){this.destroyed=true;cancelAnimationFrame(this.raf);removeEventListener('keydown',this.onKeyDown);removeEventListener('keyup',this.onKeyUp);removeEventListener('resize',this.onResize);this.renderer?.domElement.removeEventListener('pointerdown',this.onPointerDown);this.renderer?.domElement.removeEventListener('pointermove',this.onPointerMove);this.renderer?.domElement.removeEventListener('pointerup',this.onPointerUp);this.renderer?.domElement.removeEventListener('pointercancel',this.onPointerUp);this.eventQueue?.free();this.physics?.free();this.renderer?.dispose();this.renderer?.domElement.remove();this.hud?.remove();}
}
...
function createPhysicsMenuDemo(container){
  ...
  let raf=0,destroyed=false,models=null,world=null,eventQueue=null,cars=[],people=[],ragdolls=[],effects=[],deferredEffects=[],simulationTime=0,spawnTimer=0,carSerial=0,wreckCount=0;
  container.dataset.physics='rapier';container.dataset.ragdolls='0';container.dataset.wrecks='0';
  scene.fog=new THREE.Fog(0x151a1a,30,68);scene.add(new THREE.HemisphereLight(0xc6e5ff,0x28140c,2.35));
  ...
  const ground=new THREE.Mesh(new THREE.PlaneGeometry(64,34),new THREE.MeshStandardMaterial({color:0x222725,roughness:.93,metalness:.07}));ground.rotation.x=-Math.PI/2;ground.receiveShadow=true;scene.add(ground);
