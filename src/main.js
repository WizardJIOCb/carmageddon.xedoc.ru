...
class WreckrunGame {
  ...
  emitBloodDrop(pos,size,life,vel,splat=true){const material=new THREE.MeshBasicMaterial({color:Math.random()<.28?0xe01818:0x8f0000,transparent:true,opacity:.96,depthWrite:false}),drop=new THREE.Mesh(_particleGeometry,material);drop.scale.set(size*.65,size*1.9,size*.65);drop.position.copy(pos);drop.userData={vel:vel.clone(),life,maxLife:life,sharedGeometry:true,bloodDrop:true,splat,gravity:9.6,growth:0,baseSize:size,phase:Math.random()*Math.PI*2,spin:5+Math.random()*11};this.scene.add(drop);this.particles.push(drop);}
  ...
  sparkBurst(pos,count=12,normal=new THREE.Vector3()){const n=normal.clone();if(n.lengthSq()<.001)n.set(Math.random()-.5,.15,Math.random()-.5);n.normalize();const tangent=new THREE.Vector3(-n.z,0,n.x);for(let i=0;i<count;i++){const velocity=n.clone().multiplyScalar((i%2?1:-1)*(1.5+Math.random()*6)).addScaledVector(tangent,(Math.random()-.5)*12);velocity.y=1.5+Math.random()*8;this.emitParticle(pos.clone().addScaledVector(n,(Math.random()-.5)*.22),i%4?0xffb21a:0xffffff,.026+Math.random()*.045,.22+Math.random()*.5,velocity);}const material=new THREE.SpriteMaterial({map:getParticleTexture('fire'),color:0xffefad,transparent:true,opacity:.92,depthWrite:false,blending:THREE.AdditiveBlending}),flash=new THREE.Sprite(material);flash.position.copy(pos);flash.scale.setScalar(.32+Math.min(.5,count*.015));flash.userData={vel:new THREE.Vector3(),life:.1,maxLife:.1,baseOpacity:.92,gravity:0,growth:3.4};this.scene.add(flash);this.particles.push(flash);}
  bloodBurst(pos,count=24,direction=new THREE.Vector3()){const forward=direction.clone().setY(0);if(forward.lengthSq()<.01)forward.set(Math.random()-.5,0,Math.random()-.5);forward.normalize();const side=new THREE.Vector3(-forward.z,0,forward.x),origin=pos.clone().add(new THREE.Vector3(0,1,0));for(let i=0;i<count;i++){const velocity=forward.clone().multiplyScalar(3+Math.random()*9).addScaledVector(side,(Math.random()-.5)*8);velocity.y=2.4+Math.random()*8.5;this.emitBloodDrop(origin.clone().add(new THREE.Vector3((Math.random()-.5)*.45,Math.random()*.55,(Math.random()-.5)*.45)),.045+Math.random()*.13,.75+Math.random()*.75,velocity,i%2===0);}for(let i=0;i<5;i++){const distance=.25+i*(.42+Math.random()*.35),point=pos.clone().addScaledVector(forward,distance).addScaledVector(side,(Math.random()-.5)*(1.1+i*.22));this.spawnBloodDecal(point,forward,.62+Math.random()*.75,18+Math.random()*9);}}
  bloodContact(pos,direction,force=3000){const forward=direction.clone().setY(0);if(forward.lengthSq()<.001)forward.set(0,0,1);forward.normalize();const side=new THREE.Vector3(-forward.z,0,forward.x),count=Math.min(18,7+Math.round(force/4200));for(let i=0;i<count;i++){const velocity=forward.clone().multiplyScalar(1.5+Math.random()*6).addScaledVector(side,(Math.random()-.5)*5);velocity.y=1.2+Math.random()*5.5;this.emitBloodDrop(pos.clone().add(new THREE.Vector3((Math.random()-.5)*.3,.18+Math.random()*.38,(Math.random()-.5)*.3)),.035+Math.random()*.09,.55+Math.random()*.6,velocity,i%3===0);}for(let i=0;i<(force>9000?2:1);i++)this.spawnBloodDecal(pos.clone().addScaledVector(side,(Math.random()-.5)*.5),forward,.32+Math.random()*.35,12+Math.random()*7);this.renderer.domElement.dataset.ragdollBlood=String(+(this.renderer.domElement.dataset.ragdollBlood||0)+1);}
  ...
  updateParticles(dt){for(let i=this.particles.length-1;i>=0;i--){const p=this.particles[i];p.userData.life-=dt;p.userData.vel.y-=(p.userData.gravity??(p.isSprite?.45:2.5))*dt;p.position.addScaledVector(p.userData.vel,dt);if(p.userData.bloodDrop){p.userData.phase+=dt*p.userData.spin;const velocityDirection=p.userData.vel.clone();if(velocityDirection.lengthSq()>.001){const speed=velocityDirection.length(),align=new THREE.Quaternion().setFromUnitVectors(_up,velocityDirection.normalize()),roll=new THREE.Quaternion().setFromAxisAngle(velocityDirection,p.userData.phase);p.quaternion.copy(align).multiply(roll);const base=p.userData.baseSize,stretch=1.35+Math.min(2.2,speed*.085);p.scale.set(base*(.58+Math.sin(p.userData.phase)*.08),base*stretch,base*(.58-Math.sin(p.userData.phase)*.08));}if(p.position.y<=.07&&p.userData.vel.y<0){if(p.userData.splat)this.spawnBloodDecal(p.position,p.userData.vel,.16+Math.random()*.22,13+Math.random()*7);p.userData.life=0;}}p.material.opacity=Math.max(0,p.userData.life/p.userData.maxLife)*(p.userData.baseOpacity??1);if(!p.userData.bloodDrop)p.scale.addScalar(dt*(p.userData.growth??(p.isSprite?1.15:.35)));if(p.userData.life<=0){this.scene.remove(p);if(!p.userData.sharedGeometry)p.geometry?.dispose();p.material?.dispose();this.particles.splice(i,1);}}}
  ...
  updateHUD(){
    ...
    const mapX=x=>THREE.MathUtils.clamp(50+x/1.75,4,96),mapZ=z=>THREE.MathUtils.clamp(50+z/1.45,4,96),markerOffset=Math.PI/2,playerMarker=this.hud.querySelector('.map-player'),playerYaw=new THREE.Euler().setFromQuaternion(this.player.quaternion,'YXZ').y;if(playerMarker){playerMarker.style.left=mapX(this.player.position.x)+'%';playerMarker.style.top=mapZ(this.player.position.z)+'%';playerMarker.style.transform=`translate(-50%,-50%) rotate(${playerYaw+markerOffset}rad)`;}this.opponents.forEach((o,i)=>{const el=this.hud.querySelector(`[data-map-enemy="${i}"]`);if(!el)return;if(o.userData.dead){el.style.display='none';return;}const enemyYaw=new THREE.Euler().setFromQuaternion(o.quaternion,'YXZ').y;el.style.display='block';el.style.left=mapX(o.position.x)+'%';el.style.top=mapZ(o.position.z)+'%';el.style.transform=`translate(-50%,-50%) rotate(${enemyYaw+markerOffset}rad)`;});
  }
