...
class WreckrunGame {
  constructor(level){
    ...
    this.ragdolls=[];this.physicsDebris=[];this.destructibles=[];this.navigationObstacles=[];this.burningCars=[];this.colliderVehicles=new Map();this.colliderDestructibles=new Map();this.elapsed=0;this.kills=0;this.wrecks=0;this.lap=0;this.checkpoint=0;this.ended=false;this.paused=false;this.lastShot=-10;this.shake=0;this.accumulator=0;this.playerDeathPending=false;this.deathModalAt=Infinity;this.cameraOrbitYaw=0;this.cameraOrbitPitch=0;this.cameraDragging=false;this.lastCameraInput=-10;this.boostIntensity=0;this.boostEmit=0;
  }
  ...
  bindEvents(){
    ...
    this.onPointerMove=e=>{if(!this.cameraDragging)return;const dx=e.clientX-this.pointerX,dy=e.clientY-this.pointerY;this.pointerX=e.clientX;this.pointerY=e.clientY;this.cameraOrbitYaw=THREE.MathUtils.euclideanModulo(this.cameraOrbitYaw+dx*.006+Math.PI,Math.PI*2)-Math.PI;this.cameraOrbitPitch=THREE.MathUtils.clamp(this.cameraOrbitPitch+dy*.0045,-.38,.62);this.lastCameraInput=this.elapsed;};
  ...
  updatePlayer(dt){
    ...
    const boosting=(this.keys.ShiftLeft||this.keys.ShiftRight)&&forward&&d.nitro>0;this.boostIntensity=THREE.MathUtils.damp(this.boostIntensity,boosting?1:0,boosting?7:3.2,dt);this.boostEmit-=dt;if(boosting){d.nitro=Math.max(0,d.nitro-23*dt);if(this.boostEmit<=0){this.boostEmit=.024;this.emitBoostFlames(this.player);}}else d.nitro=Math.min(100,d.nitro+4.8*dt);if(this.boostLight){this.boostLight.intensity=this.boostIntensity*14;this.boostLight.position.copy(this.player.position).add(new THREE.Vector3(0,.55,-2.35).applyQuaternion(this.player.quaternion));}
  ...
  ragdollPedestrian(p,impactDirection=null,impactStrength=null,applyRecoil=true){
    if(p.userData.dead)return;p.userData.dead=true;this.kills++;const f=impactDirection?.clone().setY(0).normalize()||forwardVector(this.player.userData.vehicle.body,new THREE.Vector3()),speed=Math.max(8,Math.abs(this.player.userData.speed)),model=p.userData.model;p.userData.mixer.stopAllAction();p.updateMatrixWorld(true);model.updateMatrixWorld(true);this.scene.attach(model);p.visible=false;
      const strength=Math.min(9.5,impactStrength??(3.2+speed*.24)),impulse=f.clone().multiplyScalar(strength);impulse.y=impactStrength?Math.min(3.2,strength*.26):2.4+Math.min(1.35,speed*.04);this.ragdolls.push(createRagdoll(this.physics,this.scene,p.position,impulse,{skin:p.userData.skin,cloth:p.userData.cloth},model));
  ...
  breakObstacle(obstacle,direction,source){
    ...
    if(isPole){
      const horizontal=new THREE.Vector3(sourceVelocity.x,0,sourceVelocity.z),speed=Math.min(24,horizontal.length());if(horizontal.lengthSq()<.01)horizontal.copy(direction).multiplyScalar(8);else horizontal.setLength(speed);const velocity=horizontal.multiplyScalar(.72).addScaledVector(direction,3.2);velocity.y=2.1;this.physicsDebris.push({visual:obstacle.visual,visualOnly:true,velocity,angularVelocity:new THREE.Vector3(direction.z*4.2,.12,-direction.x*4.2),life:6,groundY:.16});
      for(let i=0;i<5;i++)this.emitParticle(position.clone().add(new THREE.Vector3(0,.35,0)),0x454b49,.055+Math.random()*.07,.35+Math.random()*.35,new THREE.Vector3((Math.random()-.5)*4,1+Math.random()*3,(Math.random()-.5)*4));this.sparkBurst(position,7);this.shake=Math.max(this.shake,.16);return;
    }
...
function createPhysicsMenuDemo(container){
  ...
  let raf=0,destroyed=false,models=null,world=null,eventQueue=null,cars=[],people=[],ragdolls=[],effects=[],simulationTime=0,spawnTimer=0,carSerial=0,wreckCount=0;
